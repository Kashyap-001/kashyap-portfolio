...
/**
 * Ported from inspira-ui's FluidCursor.vue (WebGL fluid simulation,
 * https://inspira-ui.com/docs/en/components/cursors/fluid-cursor) — the
 * simulation itself is vanilla WebGL, only the Vue lifecycle shell changed.
 * Color hue is constrained to the site's green accent instead of the
 * original's full random hue range.
 */
export function FluidCursor({ className }: FluidCursorProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const isDesktopWidth = useMediaQuery('(min-width: 700px)')
  ...
  useEffect(() => {
    ...
    const simResolution = isDesktopWidth ? 128 : 96
    const dyeResolution = isDesktopWidth ? 1440 : 720
    ...
    function splatPointer(pointer: ReturnType<typeof pointerPrototype>) {
      const dx = pointer.deltaX * config.SPLAT_FORCE
      const dy = pointer.deltaY * config.SPLAT_FORCE
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color)
    }

    function clickSplat(pointer: ReturnType<typeof pointerPrototype>) {
    ...
    function updatePointerMoveData(pointer: ReturnType<typeof pointerPrototype>, posX: number, posY: number, color: ColorRGB) {
      pointer.prevTexcoordX = pointer.texcoordX
      pointer.prevTexcoordY = pointer.texcoordY
      pointer.texcoordX = posX / canvas!.width
      pointer.texcoordY = 1 - posY / canvas!.height
    ...
    function updatePointerUpData(pointer: ReturnType<typeof pointerPrototype>) {
      pointer.down = false
    }
    ...
    // Constrained to the site's green accent hue (~0.33 on the 0-1 wheel)
    // instead of the original's full random hue range.
    ...
    cleanupFns.push(() => document.body.removeEventListener('mousemove', handleFirstMouseMove))
    ...
    cleanupFns.push(() => document.body.removeEventListener('touchstart', handleFirstTouchStart))
  ...
  }, [prefersReducedMotion, isDesktopWidth])
